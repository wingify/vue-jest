...
function getVueJestTsConfigFile (globals) {
  return globals && globals['vue-jest'] ? globals['vue-jest'] : undefined
}

function processScript (scriptPart, globals) {
  if (!scriptPart) {
    return { code: '' }
  }

  if (/^typescript|tsx?$/.test(scriptPart.lang)) {
    return compileTypescript(scriptPart.content, getVueJestTsConfigFile(globals).tsConfigFile)
  }
...
module.exports = function (src, path, jestConfig) {
  ...
  const result = processScript(parts.script, jestConfig.globals)
